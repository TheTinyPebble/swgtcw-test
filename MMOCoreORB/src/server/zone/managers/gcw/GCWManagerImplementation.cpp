...
void GCWManagerImplementation::scheduleBaseDestruction(BuildingObject* building, CreatureObject* creature) {
	if (isBaseVulnerable(building) && !hasDestroyTask(building->getObjectID()) ) {
		...
		if (building->getFaction() == Factions::FACTIONREBEL){
			building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, "\\#234BED ATTENTION SEPARATISTS, YOUR BASE IS UNDER ATTACK");
			building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, zBroadcast.toString());
		} else if (building->getFaction() == Factions::FACTIONIMPERIAL){
			building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, "\\#EEEDEF ATTENTION REPUBLIC MEMBERS, YOUR BASE IS UNDER ATTACK");
			building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, zBroadcast.toString());
		}
...
void GCWManagerImplementation::doBaseDestruction(BuildingObject* building) {
	...
	if (baseType == STATICFACTIONBASE) {
		...
		if (owner != nullptr && owner->isPlayerCreature()) {
			...
			if (building->getFaction() == Factions::FACTIONREBEL){
				building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, "\\#234BED ATTENTION SEPARATISTS, YOUR BASE HAS BEEN DESTROYED!!");
			} else if (building->getFaction() == Factions::FACTIONIMPERIAL){
				building->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, "\\#EEEDEF ATTENTION REPUBLIC MEMBERS, YOUR BASE HAS BEEN DESTROYED!!");
			}
		}
	}
