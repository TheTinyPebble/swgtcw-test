...
void SuiManager::handleCharacterBuilderSelectItem(CreatureObject* player, SuiBox* suiBox, uint32 cancel, Vector<UnicodeString>* args) {
	...
	if (node->hasChildNodes()) {
		...
		if (templatePath.indexOf(".iff") < 0) { // Non-item selections
			...
			} else if (templatePath == "unlock_jedi_tank") {
				PlayerManager* pman = zserv->getPlayerManager();
				pman->grantJediTank(player);

			} else if (templatePath == "unlock_jedi_healer") {
				PlayerManager* pman = zserv->getPlayerManager();
				pman->grantJediHealer(player);

			} else if (templatePath == "unlock_jedi_powers") {
				PlayerManager* pman = zserv->getPlayerManager();
				pman->grantJediPowers(player);

			} else if (templatePath == "unlock_jedi_sabers") {
				PlayerManager* pman = zserv->getPlayerManager();
				pman->grantJediSabers(player);
