	...
	int doQueueCommand(CreatureObject* player, const uint64& target, const UnicodeString& arguments) const {
		...
		for (int i = 0; i < ghost->getActivePetsSize(); ++i) {
			...
			if( controlDevice->getPetType() == PetManager::CREATUREPET ) {
				...
				int healthBuff = pet->getBaseHAM(CreatureAttribute::HEALTH) * 0.35;
				int actionBuff = pet->getBaseHAM(CreatureAttribute::ACTION) * 0.35;
				int mindBuff = pet->getBaseHAM(CreatureAttribute::MIND) * 0.35;
